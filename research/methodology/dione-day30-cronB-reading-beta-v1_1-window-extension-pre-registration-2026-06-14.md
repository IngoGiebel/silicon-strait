# Day-30 cron-B — Reading-β v1.1 (prior-densification) Pre-Registration — **FILED**

> **STATUS: FILED — Sprint-16 boundary, 2026-06-19 (Day-35 cron-A, 06:00 CEST).** Authored as runway at Day-30 cron-B (2026-06-14 11:00 CEST); §4 feasibility cells hardened + ROK reason corrected at Day-31 cron-A (2026-06-15, commit `38833bd`). **Filed unchanged at the Sprint-16 boundary per the pre-registered default: DEFER β-v1** — not constructible on the registered Day-1..Day-24 window (Day-29 prior table `9f9aa39`: all 9 substrates low-power, n ≤ 1 < min-n = 5, zero emittable verdicts) — **and FILE this prior-densification amendment** (as-of-truncation backfill, §3). The §4 feasibility cells and the §5 degeneracy floor ε = 0.01 are **byte-frozen** from runway commit `38833bd` (no-post-hoc-tuning guard, §7); the only changes at filing are this status/identity conversion plus a provenance-only §5 rationale wording correction (ε **value** unchanged), audited by Nisaba 🌾 job `cc5f2346` (CONCUR-WITH-NOTES, 2026-06-18 06:06 CEST). **Filing ≠ execution:** this commits to the densification *method* + degeneracy gate; the as-of-truncation backfill compute and its attached Nisaba sync audit are downstream Sprint-16 work, gated to the data-window unfreeze (still frozen 2022-10-04..2026-05-29; the chip-export-curb forward catalyst remains pre-enactment as of 2026-06-19).
...
**Pre-registered gate (decide BEFORE looking at any live Day-25+ δ):** for each backfilled substrate, require the prior inter-decile spread `pctile_90 − pctile_10 ≥ ε`, with **ε fixed here, not post-hoc**, at `ε = 0.01` δ-units — a predeclared minimum-meaningful-spread floor set at ≈ 1.3× the smallest observed Day-24 cross-substrate δ magnitude (|ROK| = 0.0077), frozen before any backfill so it cannot be tuned to admit a desired substrate. *(Provenance-only wording correction at filing, Nisaba audit `cc5f2346`: the prior draft's "one third of |ROK|" gloss was arithmetically loose — ⅓·0.0077 ≈ 0.0026, not 0.01 — so the one-third claim is dropped; the **ε = 0.01 value is byte-unchanged** and the governing rationale is the predeclared 0.01 floor itself, ≈ 1.3× |ROK|.)* A substrate whose backfilled prior spread < ε is **degeneracy-flagged**, verdict suppressed exactly as a low-power substrate, and logged. If *all* backfillable substrates fail the gate, **β-v1.1 also defers** and the fallback is §2-option-1 forward-cadence accrual filed as **β-v1.2** with its own pre-registration — not a retrofit of v1.1.

## §6 What this pre-registration does NOT commit (mirrors β-v1 §5)

- Does not execute the backfill: filing commits to the densification *method* + §5 degeneracy gate only; the as-of-truncation compute (+ its attached Nisaba sync audit) is downstream Sprint-16 work, gated to the data-window unfreeze.
...
- **No post-hoc tuning:** the as-of-truncation methodology (= β-v1 §3.1–§3.5 verbatim) and the §5 degeneracy floor ε = 0.01 were frozen at runway commit `38833bd` (Day-31 cron-A), before any backfill is run; the filing carries the value byte-identical (only the §5 *rationale wording* was corrected, audit `cc5f2346`).
...
- **Provenance:** β-v1 pre-reg `29f8c8e`; prior table `9f9aa39`; Day-24 sole sweep `dione-day24-cronC-output.json` (`bd31a40`); §4 verification data `dione-day24-cronB-data/tickers_wide.csv` + `dione-day25-cronB-data/tickers_005930ks.csv`. Authored Day-30 cron-B `silicon-strait/trunk` 2026-06-14; §4 cells verified + ROK reason corrected Day-31 cron-A 2026-06-15 (runway commit `38833bd`, byte-frozen here). **FILED at the Sprint-16 boundary, Day-35 cron-A 2026-06-19**, with a provenance-only §5 ε-rationale wording correction (Nisaba audit `cc5f2346`, CONCUR-WITH-NOTES; ε = 0.01 value unchanged). Filing commit recorded in the project state ledger (`dione-gww3-china-taiwan-state.json`).
